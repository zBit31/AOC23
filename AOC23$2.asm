...
*
         ...
         SR    R9,R9          Clear R9 - Will have the product
...
RED_FOUND EQU *
         ...
         CLC   0(2,R6),REDMAX   compare with max red cubes
         BNH   NOT_REDMAX       not red max
         MVC   REDMAX,0(R6)     >max, move it to red max
NOT_REDMAX EQU *
         LA    R6,L'RECIN(0,R5)         Reset End of scan
         LR    R5,R2               Reset start of scan from saved R2
...
GREEN_FOUND EQU *
         ...
         PUT   FILEOUT,RECOUT  print
         CLC   0(2,R6),GREENMAX   compare with max green cubes
         BNH   NOT_GREENMAX       not green max
         MVC   GREENMAX,0(R6)     >max, move it to green max
NOT_GREENMAX EQU *
         LA    R6,L'RECIN(0,R5)         Reset End of scan
         LR    R5,R2               Reset start of scan from saved R2
...
BLUE_FOUND EQU *
         ...
         PUT   FILEOUT,RECOUT  print
         CLC   0(2,R6),BLUEMAX   compare with max blue cubes
         BNH   NOT_BLUEMAX        not blue max
         MVC   BLUEMAX,0(R6)     >max, move it to blue max
NOT_BLUEMAX EQU *
         LA    R6,L'RECIN(0,R5)         Reset End of scan
         LR    R5,R2               Reset start of scan from saved R2
...
*
         ...
         MVC   RECOUT+60(6),REDMAX  spit out for debugging

         PACK  CPACKED,REDMAX
         CVB   R9,CPACKED      Load on R9 the max red
         PACK  CPACKED,GREENMAX 
         CVB   R0,CPACKED      Load on R0 the max green
         MSR   R9,R0           Multiply
         PACK  CPACKED,BLUEMAX
...
GREENMAX DS    CL2      Max green will be kept here
BLUEMAX  DS    CL2      Max blue will be kept here
* Record out
